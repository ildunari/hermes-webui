...
| `HERMES_WEBUI_POLL_INTERVAL` | `5` | Gateway session-watcher poll interval in seconds (minimum `1`). Active subscribers use this cadence. |
| `HERMES_WEBUI_POLL_IDLE_BACKOFF` | `4` | Multiplier applied to the gateway watcher poll interval while it has no SSE subscribers (minimum `1`). A new subscriber wakes the watcher immediately. |
